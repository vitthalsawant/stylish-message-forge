...
const ContentBlocks: React.FC<ContentBlocksProps> = ({ onInsertContent }) => {
  const blocks = [
    ...
    { 
      ...
      type: 'image',
      html: '<div class="draggable-row image-block" style="margin-bottom: 20px; text-align: center; padding: 20px; border: 2px dashed #d1d5db; background-color: #f9fafb; border-radius: 8px;"><div class="upload-area" style="margin-bottom: 15px;"><input type="file" accept="image/*" style="margin-bottom: 10px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white;" onchange="handleImageChange(this)" /><br><input type="url" placeholder="Or paste image URL..." style="padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white; width: 200px;" onchange="handleImageUrlChange(this)" /></div><div class="image-display" style="display: none;"><img style="max-width: 100%; height: auto; border-radius: 8px; margin-bottom: 10px;" /><div contenteditable="true" style="font-style: italic; color: #6b7280; font-size: 14px;">Click to add caption</div></div><script>function handleImageChange(input) { const file = input.files[0]; if (file) { const reader = new FileReader(); reader.onload = function(e) { const container = input.closest(".image-block"); const uploadArea = container.querySelector(".upload-area"); const imageDisplay = container.querySelector(".image-display"); const img = imageDisplay.querySelector("img"); img.src = e.target.result; uploadArea.style.display = "none"; imageDisplay.style.display = "block"; }; reader.readAsDataURL(file); } } function handleImageUrlChange(input) { const url = input.value; if (url) { const container = input.closest(".image-block"); const uploadArea = container.querySelector(".upload-area"); const imageDisplay = container.querySelector(".image-display"); const img = imageDisplay.querySelector("img"); img.src = url; uploadArea.style.display = "none"; imageDisplay.style.display = "block"; } }</script></div>'
    },
    { 
      title: 'IMAGE BOX', 
      icon: <Layers size={24} />, 
    ...
    { 
      ...
      icon: <Film size={24} />, 
      type: 'video',
      html: '<div class="draggable-row video-block" style="margin-bottom: 20px; text-align: center; padding: 20px; border: 2px dashed #d1d5db; background-color: #f9fafb; border-radius: 8px;"><div class="upload-area" style="margin-bottom: 15px;"><input type="file" accept="video/*" style="margin-bottom: 10px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white;" onchange="handleVideoChange(this)" /><br><textarea placeholder="Or paste video embed code/URL..." style="padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white; width: 250px; height: 60px; resize: vertical;" onchange="handleVideoEmbedChange(this)"></textarea></div><div class="video-display" style="display: none;"><video controls style="max-width: 100%; height: auto; border-radius: 8px; margin-bottom: 10px;"></video><div contenteditable="true" style="color: #374151; font-size: 14px;">Click to add video caption</div></div><script>function handleVideoChange(input) { const file = input.files[0]; if (file) { const reader = new FileReader(); reader.onload = function(e) { const container = input.closest(".video-block"); const uploadArea = container.querySelector(".upload-area"); const videoDisplay = container.querySelector(".video-display"); const video = videoDisplay.querySelector("video"); video.src = e.target.result; uploadArea.style.display = "none"; videoDisplay.style.display = "block"; }; reader.readAsDataURL(file); } } function handleVideoEmbedChange(textarea) { const embedCode = textarea.value; if (embedCode) { const container = textarea.closest(".video-block"); const uploadArea = container.querySelector(".upload-area"); const videoDisplay = container.querySelector(".video-display"); videoDisplay.innerHTML = embedCode + \'<div contenteditable="true" style="color: #374151; font-size: 14px; margin-top: 10px;">Click to add video caption</div>\'; uploadArea.style.display = "none"; videoDisplay.style.display = "block"; } }</script></div>'
    },
    { 
      title: 'ICONS', 
      icon: <Square size={24} />, 
      type: 'icons',
      html: '<div class="draggable-row icon-block" style="text-align: center; margin-bottom: 20px; padding: 20px; border: 2px dashed #d1d5db; background-color: #f9fafb; border-radius: 8px;"><div class="upload-area" style="margin-bottom: 15px;"><input type="file" accept="image/*,.svg" style="margin-bottom: 10px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white;" onchange="handleIconChange(this)" /><br><input type="url" placeholder="Or paste icon URL..." style="padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white; width: 200px;" onchange="handleIconUrlChange(this)" /></div><div class="icon-display" style="display: none;"><img style="width: 64px; height: 64px; border-radius: 8px; object-fit: contain; margin-bottom: 10px;" /><div contenteditable="true" style="color: #374151; font-size: 14px;">Click to edit icon label</div></div><script>function handleIconChange(input) { const file = input.files[0]; if (file) { const reader = new FileReader(); reader.onload = function(e) { const container = input.closest(".icon-block"); const uploadArea = container.querySelector(".upload-area"); const iconDisplay = container.querySelector(".icon-display"); const img = iconDisplay.querySelector("img"); img.src = e.target.result; uploadArea.style.display = "none"; iconDisplay.style.display = "block"; }; reader.readAsDataURL(file); } } function handleIconUrlChange(input) { const url = input.value; if (url) { const container = input.closest(".icon-block"); const uploadArea = container.querySelector(".upload-area"); const iconDisplay = container.querySelector(".icon-display"); const img = iconDisplay.querySelector("img"); img.src = url; uploadArea.style.display = "none"; iconDisplay.style.display = "block"; } }</script></div>'
    },
    { 
      title: 'MENU', 
    ...
    { 
      ...
      type: 'sticker',
      html: '<div class="draggable-row sticker-block" style="text-align: center; margin-bottom: 20px; padding: 20px; border: 2px dashed #d1d5db; background-color: #f9fafb; border-radius: 8px;"><div class="upload-area" style="margin-bottom: 15px;"><input type="file" accept="image/*" style="margin-bottom: 10px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white;" onchange="handleStickerChange(this)" /><br><input type="url" placeholder="Or paste sticker URL..." style="padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white; width: 200px;" onchange="handleStickerUrlChange(this)" /></div><div class="sticker-display" style="display: none;"><img style="max-width: 150px; max-height: 150px; border-radius: 12px; object-fit: cover; transform: rotate(-2deg); margin-bottom: 10px;" /><div contenteditable="true" style="color: #374151; font-size: 14px;">Click to edit sticker caption</div></div><script>function handleStickerChange(input) { const file = input.files[0]; if (file) { const reader = new FileReader(); reader.onload = function(e) { const container = input.closest(".sticker-block"); const uploadArea = container.querySelector(".upload-area"); const stickerDisplay = container.querySelector(".sticker-display"); const img = stickerDisplay.querySelector("img"); img.src = e.target.result; uploadArea.style.display = "none"; stickerDisplay.style.display = "block"; }; reader.readAsDataURL(file); } } function handleStickerUrlChange(input) { const url = input.value; if (url) { const container = input.closest(".sticker-block"); const uploadArea = container.querySelector(".upload-area"); const stickerDisplay = container.querySelector(".sticker-display"); const img = stickerDisplay.querySelector("img"); img.src = url; uploadArea.style.display = "none"; stickerDisplay.style.display = "block"; } }</script></div>'
    },
    { 
      title: 'GIF', 
      icon: <FileImage size={24} />, 
      type: 'gif',
      html: '<div class="draggable-row gif-block" style="text-align: center; margin-bottom: 20px; padding: 20px; border: 2px dashed #d1d5db; background-color: #f9fafb; border-radius: 8px;"><div class="upload-area" style="margin-bottom: 15px;"><input type="file" accept=".gif,image/gif" style="margin-bottom: 10px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white;" onchange="handleGifChange(this)" /><br><input type="url" placeholder="Or paste GIF URL..." style="padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white; width: 200px;" onchange="handleGifUrlChange(this)" /><div style="font-size: 12px; color: #6b7280; margin-top: 5px;">Only .gif format accepted</div></div><div class="gif-display" style="display: none;"><img style="max-width: 200px; max-height: 150px; border-radius: 8px; object-fit: cover; margin-bottom: 10px;" /><div contenteditable="true" style="color: #374151; font-size: 14px;">Click to add GIF caption</div></div><script>function handleGifChange(input) { const file = input.files[0]; if (file && file.type === "image/gif") { const reader = new FileReader(); reader.onload = function(e) { const container = input.closest(".gif-block"); const uploadArea = container.querySelector(".upload-area"); const gifDisplay = container.querySelector(".gif-display"); const img = gifDisplay.querySelector("img"); img.src = e.target.result; uploadArea.style.display = "none"; gifDisplay.style.display = "block"; }; reader.readAsDataURL(file); } else if (file) { alert("Please select a valid GIF file."); } } function handleGifUrlChange(input) { const url = input.value; if (url && url.toLowerCase().includes(".gif")) { const container = input.closest(".gif-block"); const uploadArea = container.querySelector(".upload-area"); const gifDisplay = container.querySelector(".gif-display"); const img = gifDisplay.querySelector("img"); img.src = url; uploadArea.style.display = "none"; gifDisplay.style.display = "block"; } }</script></div>'
    }
  ];
